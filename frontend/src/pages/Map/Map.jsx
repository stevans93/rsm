...
function Map() {
  ...
  const [selectedMunicipalityInfo, setSelectedMunicipalityInfo] =
    useState(null);
  ...
  const handlePathClick = (event, pathId, title) => {
    const clickedMunicipality = municipalities?.filter(
      (municipality) => municipality.district === title
    );
  ...
  return (
    <div>
      <div className="flex overflow-x-scroll relative w-full px-[20px] mt-[20px]">
            ...
            {MapData.map((path) => (
              <g key={path.id}>
                ...
                <path
                  ...
                  stroke={path.stroke}
                  fill={
                    hoveredPath === path.id || selectedPathId === path.id
                      ? "#A3C8DB"
                      : "#fff"
                  }
                  onClick={(event) => {
        ...
        <div
          ref={sidebarRef}
          className={`w-[70%] md:block fixed left-[15%] md:w-[20%] md:static top-[25%] ${
            !sidebar ? "hidden" : ""
          }`}
        >
          <MapSideBar
      ...
      {isInfoVisible && (
        <div
          className={`absolute top-[60%] left-[50%] translate-y-[-50%] translate-x-[-50%] `}
        >
          <InfoAboutTheCity
            handleToggleInfo={handleToggleInfo}
